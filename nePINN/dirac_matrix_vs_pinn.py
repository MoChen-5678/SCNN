...
边界条件嵌入网络结构 (r^{l+1} * (e^{-βr} + C), β共享/C独立).
Rayleigh商从波函数计算物理能量.
...
class DiracPINNSolver:
    ...
    def train(self, E_init_MeV=-60.0, target_nodes=0,
        ...
        if load_model and os.path.exists(load_model):
            ...
            if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
                net.load_state_dict(checkpoint['state_dict'], strict=False)  # strict=False: 兼容旧模型缺beta
        ...
        if live_plot:
            ...
            ax_comp, ax_beta = axes[1][0], axes[1][1]
            ...
            # Params
            ax_beta.set_title('Boundary Params'); ax_beta.set_xlabel('Epoch')
            ln_beta, = ax_beta.plot([], [], 'm-', lw=1.5, label=r'$\beta$')
        ...
        for epoch in range(max_epochs):
            ...
            g, f = net(r_input, kappa=self.kappa, dr=DR)
            ...
            # 边界条件损失 (RHF论文: ω_BI=0, 边界已嵌入网络结构 e^{-βr}·r^{l+1})
            if w_bc > 0:
            ...
            loss_total = (w_pde * loss_pde + w_bc * loss_bc + w_ortho * loss_ortho + 0*loss_norm)
            loss_total.backward()
            ...
            if epoch % print_every == 0 or epoch == max_epochs - 1:
                beta_val = torch.nn.functional.softplus(net.beta).item()
                print(f"  Ep {epoch:5d}: L={loss_total.item():.2e}  "
                      f"pde={loss_pde.item():.2e} bc={loss_bc.item():.2e} "
                      f"ortho={loss_ortho.item():.2e} norm={loss_norm.item():.2e}  "
                      f"E_ray={E_ray_hc.item():+.4f} β={beta_val:.4f}")
            ...
            if live_plot and (epoch % plot_update_interval == 0 or epoch == max_epochs - 1):
                ...
                if not hasattr(self, '_param_history'):
                    self._param_history = {'beta': []}
                self._param_history['beta'].append(beta_val)
                n_p = len(self._param_history['beta'])
                p_eps = list(range(n_p))
                ln_beta.set_data(p_eps, self._param_history['beta'])
                ax_beta.relim(); ax_beta.autoscale_view()
            ...
            history.append({'epoch': epoch, 'loss': loss_total.item(), 'E_MeV': E_ray_hc.item()})
        ...
        # 最终用 Rayleigh 商算物理能量
        with torch.no_grad():
            g_final, f_final = net(r_input, kappa=self.kappa, dr=DR)
    ...
    def get_wavefunction(self, r_eval=None):
        ...
        g, f = self.net(rt, kappa=self.kappa, dr=DR)
...
def solve_state(args):
    ...
    # ★ 激发态：强制禁用迁移学习 + 内嵌正交(硬约束) + 外部正交(安全网)
    is_excited = len(ref_wavefunctions) > 0
    if is_excited:
        args.load_model = None  # 不用基态模型做起点
        w_ortho_val = 1.0      # ★ 硬约束已内嵌到forward(), 外部只需极小安全网
        print(f'  [Excited state] load_model=DISABLED, embedded Gram-Schmidt ON, w_ortho(safety)={w_ortho_val}')
    else:
        w_ortho_val = 0.0
    ...
    E_pinn, history = solver.train(
        ...
        w_pde=20.0, w_bc=0, w_ortho=w_ortho_val,
    )
...
def main():
    ...
    if base_result and base_result[1]:  # base_result[1] = all_wf_paths (list)
        ...
        if args.load_model is None:
            args.load_model = ''  # 明确禁用迁移学习
        print(f'\n{"="*60}')
        print(f'Now solving target state: {args.state}  [NO transfer learning]')
        print(f'{"="*60}')
