...
!--- ILAENV DGEMM DGEMV DSYR DSYMV XERBLA IDAMAX IPARMQ IEEECK LSAME DCOPY DDOT DNRM2 DSWAP DSCAL
!--- DLAMCH DLACPY DLASCL DLARFB DLARFT DTRMV DLASET DLASRT DLANST, DLASSQ, DLAE2, DLAEV2, DLARTG
!--- DLASR DLAPY2 DLANSY DLARF DLAMRG DLAED0-9 DLAEDA DLAMC1-5 DAXPY DSYR2 DSYR2K DLARFG DLATRD
!--- DTRMM DGER DSTEQR DROT
Contains
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
!
...
!
!     .. Local Scalars ..
      INTEGER    I, IC, IZ, NB, NBMIN, NX, nik8
...
!
!     .. Scalar Arguments ..
      INTEGER    IHI, ILO, ISPEC, LWORK, N, IPARMQ
      CHARACTER          NAME*( * ), OPTS*( * )
!
!  ================================================================
!     .. Parameters ..
...
!
      ...
      ELSE IF( ITYPE.GE.4 ) THEN
         ...
         ELSE IF( KU.LT.0 .OR. KU.GT.MAX( N-1, 0 ) .OR. ( ( ITYPE.EQ.4 .OR. ITYPE.EQ.5 ) .AND. KL.NE.KU ) )  THEN
            INFO = -3
         ELSE IF( ( ITYPE.EQ.4 .AND. LDA.LT.KL+1 ) .OR. ( ITYPE.EQ.5 .AND. LDA.LT.KU+1 ) .OR. &
...
!
               nik8 = 1
               DO 10 J = 1, K
                  CALL DCOPY( N, C( J, 1 ), LDC, WORK( 1, J ), nik8 )
   10          CONTINUE
!
!              W := W * V1
!
...
!                 W := W + C2'*V2
!
                  CALL DGEMM( 'Transpose', 'No transpose', N, K, M-K, ONE, C( K+1, 1 ), LDC, V( K+1, 1 ), LDV, &
     &                        ONE, WORK, LDWORK )
               END IF
!
!              W := W * T'  or  W * T
!
               CALL DTRMM( 'Right', 'Upper', TRANST, 'Non-unit', N, K, ONE, T, LDT, WORK, LDWORK )
!
!              C := C - V * W'
!
               IF( M.GT.K ) THEN
...
!                 C2 := C2 - V2 * W'
!
                  CALL DGEMM( 'No transpose', 'Transpose', M-K, N, K, -ONE, V( K+1, 1 ), LDV, WORK, LDWORK, ONE, &
     &                        C( K+1, 1 ), LDC )
               END IF
!
!              W := W * V1'
!
               CALL DTRMM( 'Right', 'Lower', 'Transpose', 'Unit', N, K,  ONE, V, LDV, WORK, LDWORK )
!
!              C1 := C1 - W'
!
               DO 30 J = 1, K
...
!
                nik8 = 1
                DO 40 J = 1, K
                  CALL DCOPY( M, C( 1, J ), nik8, WORK( 1, J ), nik8 )
   40          CONTINUE
!
!              W := W * V1
!
...
!                 W := W + C2 * V2
!
                  CALL DGEMM( 'No transpose', 'No transpose', M, K, N-K,  ONE, C( 1, K+1 ), LDC, V( K+1, 1 ), LDV, &
     &                        ONE, WORK, LDWORK )
               END IF
!
!              W := W * T  or  W * T'
!
               CALL DTRMM( 'Right', 'Upper', TRANS, 'Non-unit', M, K, ONE, T, LDT, WORK, LDWORK )
!
!              C := C - W * V'
!
               IF( N.GT.K ) THEN
...
!                 C2 := C2 - W * V2'
!
                  CALL DGEMM( 'No transpose', 'Transpose', M, N-K, K,  -ONE, WORK, LDWORK, V( K+1, 1 ), LDV, ONE, &
     &                        C( 1, K+1 ), LDC )
               END IF
!
!              W := W * V1'
!
               CALL DTRMM( 'Right', 'Lower', 'Transpose', 'Unit', M, K, ONE, V, LDV, WORK, LDWORK )
!
!              C1 := C1 - W
!
               DO 60 J = 1, K
...
!
               nik8 = 1
               DO 70 J = 1, K
                  CALL DCOPY( N, C( M-K+J, 1 ), LDC, WORK( 1, J ), nik8 )
   70          CONTINUE
!
!              W := W * V2
!
...
!              W := W * V2'
!
               CALL DTRMM( 'Right', 'Upper', 'Transpose', 'Unit', N, K, ONE, V( M-K+1, 1 ), LDV, WORK, LDWORK )
!
!              C2 := C2 - W'
!
               DO 90 J = 1, K
...
!
               nik8 = 1
               DO 100 J = 1, K
                  CALL DCOPY( M, C( 1, N-K+J ), nik8, WORK( 1, J ), nik8 )
  100          CONTINUE
!
!              W := W * V2
!
...
!              W := W * V2'
!
               CALL DTRMM( 'Right', 'Upper', 'Transpose', 'Unit', M, K, ONE, V( N-K+1, 1 ), LDV, WORK, LDWORK )
!
!              C2 := C2 - W
!
               DO 120 J = 1, K
...
!                 W := W + C2'*V2'
!
                  CALL DGEMM( 'Transpose', 'Transpose', N, K, M-K, ONE, C( K+1, 1 ), LDC, V( 1, K+1 ), LDV, ONE, &
     &                        WORK, LDWORK )
               END IF
!
!              W := W * T'  or  W * T
...
!                 C2 := C2 - V2' * W'
!
                  CALL DGEMM( 'Transpose', 'Transpose', M-K, N, K, -ONE, V( 1, K+1 ), LDV, WORK, LDWORK, ONE, &
     &                        C( K+1, 1 ), LDC )
               END IF
!
!              W := W * V1
!
               CALL DTRMM( 'Right', 'Upper', 'No transpose', 'Unit', N, K, ONE, V, LDV, WORK, LDWORK )
!
!              C1 := C1 - W'
!
               DO 150 J = 1, K
...
!                 W := W + C2 * V2'
!
                  CALL DGEMM( 'No transpose', 'Transpose', M, K, N-K,  ONE, C( 1, K+1 ), LDC, V( 1, K+1 ), LDV, &
     &                        ONE, WORK, LDWORK )
               END IF
!
!              W := W * T  or  W * T'
...
!                 C2 := C2 - W * V2
!
                  CALL DGEMM( 'No transpose', 'No transpose', M, N-K, K,  -ONE, WORK, LDWORK, V( 1, K+1 ), LDV, ONE, &
     &                        C( 1, K+1 ), LDC )
               END IF
!
!              W := W * V1
!
               CALL DTRMM( 'Right', 'Upper', 'No transpose', 'Unit', M, K, ONE, V, LDV, WORK, LDWORK )
!
!              C1 := C1 - W
!
               DO 180 J = 1, K
...
!              W := W * V2
!
               CALL DTRMM( 'Right', 'Lower', 'No transpose', 'Unit', N,  K, ONE, V( 1, M-K+1 ), LDV, WORK, LDWORK )
!
!              C2 := C2 - W'
!
               DO 210 J = 1, K
...
!              W := W * V2
!
               CALL DTRMM( 'Right', 'Lower', 'No transpose', 'Unit', M, K, ONE, V( 1, N-K+1 ), LDV, WORK, LDWORK )
!
!              C1 := C1 - W
!
               DO 240 J = 1, K
...
!
                   nik8 = 1
                  CALL DGEMV( 'Transpose', N-I+1, I-1, -TAU( I ), V( I, 1 ), LDV, V( I, I ), nik8, ZERO, &
     &                        T( 1, I ), nik8 )
               ELSE
!
...
!
                   nik8 = 1
                  CALL DGEMV( 'No transpose', I-1, N-I+1, -TAU( I ), V( 1, I ), LDV, V( I, I ), LDV, ZERO, &
     &                        T( 1, I ), nik8 )
               END IF
               V( I, I ) = VII
!
...
!
               nik8 = 1;    nk8 = I-1
               CALL DTRMV( 'Upper', 'No transpose', 'Non-unit', nk8, T, LDT, T( 1, I ), nik8 )
               T( I, I ) = TAU( I )
            END IF
   20    CONTINUE
      ELSE
...
!
                     nik8 = 1
                     CALL DGEMV( 'Transpose', N-K+I, K-I, -TAU( I ), V( 1, I+1 ), LDV, V( 1, I ), nik8, ZERO, &
     &                           T( I+1, I ), nik8 )
                     V( N-K+I, I ) = VII
                  ELSE
                     VII = V( I, N-K+I )
...
!
                     nik8 = 1
                     CALL DGEMV( 'No transpose', K-I, N-K+I, -TAU( I ), V( I+1, 1 ), LDV, V( I, 1 ), LDV, ZERO, &
     &                           T( I+1, I ), nik8 )
                     V( I, N-K+I ) = VII
                  END IF
!
...
!   
                  nik8 = 1;   nk8 = K-I
                  CALL DTRMV( 'Lower', 'No transpose', 'Non-unit', nk8, T( I+1, I+1 ), LDT, T( I+1, I ), nik8 )
               END IF
               T( I, I ) = TAU( I )
            END IF
   40    CONTINUE
...
!
        ...
        nik8 = 1
         IF( N.GT.1 ) THEN
            CALL DLASSQ( N-1, E, nik8, SCALE, SUM )
            SUM = 2*SUM
         END IF
         CALL DLASSQ( N, D, nik8, SCALE, SUM )
...
!
         ...
         IF( LSAME( UPLO, 'U' ) ) THEN
             nik8 = 1
            DO 110 J = 2, N
               CALL DLASSQ( J-1, A( 1, J ), nik8, SCALE, SUM )
  110       CONTINUE
         ELSE
             nik8 = 1
            DO 120 J = 1, N - 1
               CALL DLASSQ( N-J, A( J+1, J ), nik8, SCALE, SUM )
  120       CONTINUE
         END IF
         SUM = 2*SUM
         nik8 = LDA+1
         CALL DLASSQ( N, A, nik8, SCALE, SUM )
...
!
             nik8 = 1
            CALL DGEMV( 'Transpose', M, N, ONE, C, LDC, V, INCV, ZERO,  WORK, nik8 )
!
!           C := C - v * w'
!
            CALL DGER( M, N, -TAU, V, INCV, WORK, nik8, C, LDC )
         END IF
      ELSE
!
!        Form  C * H
...
!
             nik8 = 1
            CALL DGEMV( 'No transpose', M, N, ONE, C, LDC, V, INCV,  ZERO, WORK, nik8 )
!
!           C := C - w * v'
!
            CALL DGER( M, N, -TAU, WORK, nik8, V, INCV, C, LDC )
         END IF
      END IF
      RETURN
!
...
!     .. Local Scalars ..
     ...
     &                   SPM2, SUBMAT, SUBPBS, TLVLS, nik8, nk8
      DOUBLE PRECISION   TEMP
!     ..
!     .. External Subroutines ..
...
!
      nik8 = 0;     nk8 = 9
      SMLSIZ = ILAENV( nk8, 'DLAED0', ' ', nik8, nik8, nik8, nik8 )
!
...
!
      IF( ICOMPQ.EQ.1 ) THEN
         ...
         DO 100 I = 1, N
            J = IWORK( INDXQ+I )
            WORK( I ) = D( J )
            CALL DCOPY( QSIZ, QSTORE( 1, J ), nik8, Q( 1, I ), nik8 )
100      CONTINUE
         nk8 = 1
         CALL DCOPY( N, WORK, nk8, D, nk8 )
      ELSE IF( ICOMPQ.EQ.2 ) THEN
          nik8 = 1
         DO 110 I = 1, N
            J = IWORK( INDXQ+I )
            WORK( I ) = D( J )
            CALL DCOPY( N, Q( 1, J ), nik8, WORK( N*I+1 ), nik8 )
110      CONTINUE
         nik8 = 1
         CALL DCOPY( N, WORK, nik8, D, nik8 )
...
!
!     .. Local Scalars ..
      INTEGER :: COLTYP, I, IDLMDA, INDX, INDXC, INDXP, IQ2, IS, IW, IZ, K, N1, N2, ZPP1, nik8, nk8
!     ..
!     .. External Subroutines ..
...
!
      nik8 = 1
      CALL DCOPY( CUTPNT, Q( CUTPNT, 1 ), LDQ, WORK( IZ ), nik8 )
      ZPP1 = CUTPNT + 1
      CALL DCOPY( N-CUTPNT, Q( ZPP1, ZPP1 ), LDQ, WORK( IZ+CUTPNT ), nik8 )
!
!     Deflate eigenvalues.
!
...
!
         ...
         N2 = N - K
         nik8 = 1;      nk8 = -1
         CALL DLAMRG( N1, N2, D, nik8, nk8, INDXQ )
      ELSE
         DO 10 I = 1, N
            INDXQ( I ) = I
   10    CONTINUE
...
!     ..
!     .. Local Scalars ..
      INTEGER :: CT, I, IMAX, IQ1, IQ2, J, JMAX, JS, K2, N1P1, N2, NJ, PJ, nik8
...
!
      IF( RHO.LT.ZERO ) THEN
         nik8 = 1; CALL DSCAL( N2, MONE, Z( N1P1 ), nik8 )
      END IF
!
...
20    CONTINUE
      nik8 = 1
      CALL DLAMRG( N1, N2, DLAMDA, nik8, nik8, INDXC )
      DO 30 I = 1, N
...
!
      nik8 = 1
      IMAX = IDAMAX( N, Z, nik8 )
      JMAX = IDAMAX( N, D, nik8 )
      EPS = DLAMCH( 'Epsilon' )
...
!
      IF( RHO*ABS( Z( IMAX ) ).LE.TOL ) THEN
         ...
         DO 40 J = 1, N
            I = INDX( J )
            CALL DCOPY( N, Q( 1, I ), nik8, Q2( IQ2 ), nik8 )
            DLAMDA( J ) = D( I )
            IQ2 = IQ2 + N
40       CONTINUE
         nik8 = 1
         CALL DLACPY( 'A', N, N, Q2, N, Q, LDQ )
         CALL DCOPY( N, DLAMDA, nik8, D, nik8 )
         GO TO 190
      END IF
!
...
!
            ...
            COLTYP( PJ ) = 4
            nik8 = 1
            CALL DROT( N, Q( 1, PJ ), nik8, Q( 1, NJ ), nik8, C, S )
            T = D( PJ )*C**2 + D( NJ )*S**2
...
!
      ...
      DO 140 J = 1, CTOT( 1 )
         JS = INDX( I )
         CALL DCOPY( N1, Q( 1, JS ), nik8, Q2( IQ1 ), nik8 )
         Z( I ) = D( JS )
         I = I + 1
         IQ1 = IQ1 + N1
140   CONTINUE
!
      ...
      DO 150 J = 1, CTOT( 2 )
         JS = INDX( I )
         CALL DCOPY( N1, Q( 1, JS ), nik8, Q2( IQ1 ), nik8 )
         CALL DCOPY( N2, Q( N1+1, JS ), nik8, Q2( IQ2 ), nik8 )
         Z( I ) = D( JS )
         I = I + 1
         IQ1 = IQ1 + N1
         IQ2 = IQ2 + N2
...
!
      ...
      DO 170 J = 1, CTOT( 4 )
         JS = INDX( I )
         CALL DCOPY( N, Q( 1, JS ), nik8, Q2( IQ2 ), nik8 )
         IQ2 = IQ2 + N
         Z( I ) = D( JS )
         I = I + 1
  170 CONTINUE
...
!
      ...
      nik8 = 1
      CALL DCOPY( N-K, Z( K+1 ), nik8, D( K+1 ), nik8 )
!
...
!     ..
!     .. Local Scalars ..
      INTEGER :: I, II, IQ2, J, N12, N2, N23, nik8
      DOUBLE PRECISION   TEMP
!     ..
!     .. External Functions ..
...
!     Compute updated W.
!
      nik8 = 1; CALL DCOPY( K, W, nik8, S, nik8 )
!
!     Initialize W(I) = Q(I,I)
!
      CALL DCOPY( K, Q, LDQ+1, W, nik8 )
      DO 60 J = 1, K
         DO 40 I = 1, J - 1
...
!     .. Parameters ..
      ...
      PARAMETER          ( ZERO = 0.0D0, ONE = 1.0D0, TWO = 2.0D0, THREE = 3.0D0, &
          & FOUR = 4.0D0, EIGHT = 8.0D0, TEN = 10.0D0 )
!     ..
!     .. Local Scalars ..
      LOGICAL            ORGATI, SWTCH, SWTCH3
      INTEGER :: II, IIM1, IIP1, IP1, ITER, J, NITER
      DOUBLE PRECISION   A, B, C, DEL, DLTLB, DLTUB, DPHI, DPSI, DW, EPS, ERRETM, &
          & ETA, MIDPT, PHI, PREW, PSI, RHOINV, TAU, TEMP, TEMP1, W
!     ..
!     .. Local Arrays ..
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
!
SUBROUTINE DLAED7( ICOMPQ, N, QSIZ, TLVLS, CURLVL, CURPBM, D, Q, LDQ, INDXQ, RHO, CUTPNT, QSTORE, &
...
!     ..
!     .. Local Scalars ..
      INTEGER :: COLTYP, CURR, I, IDLMDA, INDX, INDXC, INDXP, IQ2, IS, IW, IZ, &
                    & K, LDQ2, N1, N2, PTR, nik8, nk8
!     ..
!     .. External Subroutines ..
...
!
      IF( K.NE.0 ) THEN
         nik8 = 1; CALL DLAED9( K, nik8, K, N, D, WORK( IS ), K, RHO, WORK( IDLMDA ), WORK( IW ), &
             & QSTORE( QPTR( CURR ) ), K, INFO )
         IF( INFO.NE.0 ) GO TO 30
         IF( ICOMPQ.EQ.1 ) THEN
            CALL DGEMM( 'N', 'N', QSIZ, K, K, ONE, WORK( IQ2 ), LDQ2, QSTORE( QPTR( CURR ) ), &
                & K, ZERO, Q, LDQ )
         END IF
...
!
         N1 = K
         N2 = N - K
         nik8 = 1; nk8 = -1; CALL DLAMRG( N1, N2, D, nik8, nk8, INDXQ )
      ELSE
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
!
SUBROUTINE DLAED8( ICOMPQ, K, N, QSIZ, D, Q, LDQ, INDXQ, RHO, CUTPNT, Z, DLAMDA, Q2, LDQ2, W, PERM, &
...
!     .. Local Scalars ..
!
      INTEGER :: I, IMAX, J, JLAM, JMAX, JP, K2, N1, N1P1, N2, nik8
      DOUBLE PRECISION   C, EPS, S, T, TAU, TOL
!     ..
!     .. External Functions ..
!     INTEGER            IDAMAX
...
!
   ...
   30 CONTINUE
      I = 1
      J = CUTPNT + 1
      nik8 = 1;  CALL DLAMRG( N1, N2, DLAMDA, nik8, nik8, INDX )
      DO 40 I = 1, N
         D( I ) = DLAMDA( INDX( I ) )
         Z( I ) = W( INDX( I ) )
   40 CONTINUE
...
!
   ...
   50       CONTINUE
         ELSE
            DO 60 J = 1, N
               PERM( J ) = INDXQ( INDX( J ) )
               nik8 = 1; CALL DCOPY( QSIZ, Q( 1, PERM( J ) ), nik8, Q2( 1, J ), nik8 )
   60       CONTINUE
...
!
            ...
            GIVNUM( 2, GIVPTR ) = S
            IF( ICOMPQ.EQ.1 ) THEN
               nik8 = 1; CALL DROT( QSIZ, Q( 1, INDXQ( INDX( JLAM ) ) ), nik8, Q( 1, INDXQ( INDX( J ) ) ), nik8, C, S )
            END IF
...
!
      IF( K.LT.N ) THEN
         IF( ICOMPQ.EQ.0 ) THEN
            nik8 = 1; CALL DCOPY( N-K, DLAMDA( K+1 ), nik8, D( K+1 ), nik8 )
         ELSE
            nik8 = 1; CALL DCOPY( N-K, DLAMDA( K+1 ), nik8, D( K+1 ), nik8 )
...
!     Initialize W(I) = Q(I,I)
!
      nik8 = 1; CALL DCOPY( K, Q, LDQ+1, W, nik8 )
      DO 70 J = 1, K
         DO 50 I = 1, J - 1
            W( I ) = W( I )*( Q( I, J ) / ( DLAMDA( I )-DLAMDA( J ) ) )
   50    CONTINUE
...
!
      DO 110 J = 1, K
         DO 90 I = 1, K
            Q( I, J ) = W( I ) / Q( I, J )
   90    CONTINUE
         nik8 = 1; TEMP = DNRM2( K, Q( 1, J ), nik8 )
         DO 100 I = 1, K
            S( I, J ) = Q( I, J ) / TEMP
  100    CONTINUE
  110 CONTINUE
...
!     ..
!     .. Local Scalars ..
      INTEGER :: BSIZ1, BSIZ2, CURR, I, K, MID, PSIZ1, PSIZ2, PTR, ZPTR1, nik8
!     ..
!     .. External Subroutines ..
...
!
      ...
      DO 10 K = 1, MID - BSIZ1 - 1
         Z( K ) = ZERO
   10 CONTINUE
      nik8 = 1; CALL DCOPY( BSIZ1, Q( QPTR( CURR )+BSIZ1-1 ), BSIZ1, Z( MID-BSIZ1 ), nik8 )
      CALL DCOPY( BSIZ2, Q( QPTR( CURR+1 ) ), BSIZ2, Z( MID ), nik8 )
      DO 20 K = MID + BSIZ2, N
         Z( K ) = ZERO
   20 CONTINUE
!
...
!       Apply Givens at CURR and CURR+1
         ...
         DO 30 I = GIVPTR( CURR ), GIVPTR( CURR+1 ) - 1
            CALL DROT( nik8, Z( ZPTR1+GIVCOL( 1, I )-1 ), nik8, Z( ZPTR1+GIVCOL( 2, I )-1 ), nik8, &
                & GIVNUM( 1, I ), GIVNUM( 2, I ) )
   30    CONTINUE
         DO 40 I = GIVPTR( CURR+1 ), GIVPTR( CURR+2 ) - 1
            CALL DROT( nik8, Z( MID-1+GIVCOL( 1, I ) ), nik8, Z( MID-1+GIVCOL( 2, I ) ), nik8, &
                & GIVNUM( 1, I ), GIVNUM( 2, I ) )
   40    CONTINUE
         PSIZ1 = PRMPTR( CURR+1 ) - PRMPTR( CURR )
         PSIZ2 = PRMPTR( CURR+2 ) - PRMPTR( CURR+1 )
...
!
         ...
         IF( BSIZ1.GT.0 ) THEN
            nik8 = 1
            CALL DGEMV( 'T', BSIZ1, BSIZ1, ONE, Q( QPTR( CURR ) ), BSIZ1, ZTEMP( 1 ), nik8, ZERO, Z( ZPTR1 ), nik8 )
         END IF
         nik8 = 1; CALL DCOPY( PSIZ1-BSIZ1, ZTEMP( BSIZ1+1 ), nik8, Z( ZPTR1+BSIZ1 ), nik8 )
         IF( BSIZ2.GT.0 ) THEN
            nik8 = 1
            CALL DGEMV( 'T', BSIZ2, BSIZ2, ONE, Q( QPTR( CURR+1 ) ), BSIZ2, ZTEMP( PSIZ1+1 ), nik8, ZERO, Z( MID ), nik8 )
         END IF
         nik8 = 1
         CALL DCOPY( PSIZ2-BSIZ2, ZTEMP( PSIZ1+BSIZ2+1 ), nik8, Z( MID+BSIZ2 ), nik8 )
!
         PTR = PTR + 2**( TLVLS-K )
   70 CONTINUE
!
...
!
     ...
     &                     LDA, W( I, IW+1 ), LDW, ONE, A( 1, I ), nik8 )
               CALL DGEMV( 'No transpose', I, N-I, -ONE, W( 1, IW+1 ),&
     &                     LDW, A( I, I+1 ), LDA, ONE, A( 1, I ), nik8 )
            END IF
            IF( I.GT.1 ) THEN
!
...
!              A(1:i-2,i)
!
               nik8=1; CALL DLARFG( I-1, A( I-1, I ), A( 1, I ), nik8, TAU( I-1 ) )
               E( I-1 ) = A( I-1, I )
               A( I-1, I ) = ONE
!
!              Compute W(1:i-1,i)
!
               CALL DSYMV( 'Upper', I-1, ONE, A, LDA, A( 1, I ), nik8, &
     &                     ZERO, W( 1, IW ), nik8 )
               IF( I.LT.N ) THEN
                   nik8 = 1
                  CALL DGEMV( 'Transpose', I-1, N-I, ONE, W( 1, IW+1 ), &
     &                        LDW, A( 1, I ), nik8, ZERO, W( I+1, IW ), nik8 )
                  CALL DGEMV( 'No transpose', I-1, N-I, -ONE,       &
     &                        A( 1, I+1 ), LDA, W( I+1, IW ), nik8, ONE, &
     &                        W( 1, IW ), nik8 )
                  CALL DGEMV( 'Transpose', I-1, N-I, ONE, A( 1, I+1 ), &
     &                        LDA, A( 1, I ), nik8, ZERO, W( I+1, IW ), nik8 )
                  CALL DGEMV( 'No transpose', I-1, N-I, -ONE,           &
     &                        W( 1, IW+1 ), LDW, W( I+1, IW ), nik8, ONE,  &
     &                        W( 1, IW ), nik8 )
               END IF
               nik8 = 1
               CALL DSCAL( I-1, TAU( I-1 ), W( 1, IW ), nik8 )
               ALPHA = -HALF*TAU( I-1 )*DDOT( I-1, W( 1, IW ), nik8, A( 1, I ), nik8 )
               CALL DAXPY( I-1, ALPHA, A( 1, I ), nik8, W( 1, IW ), nik8 )
            END IF
!
   10    CONTINUE
      ELSE
...
!
     ...
     &                  LDA, W( I, 1 ), LDW, ONE, A( I, I ), nik8 )
            CALL DGEMV( 'No transpose', N-I+1, I-1, -ONE, W( I, 1 ), &
     &                  LDW, A( I, 1 ), LDA, ONE, A( I, I ), nik8 )
            IF( I.LT.N ) THEN
!
!              Generate elementary reflector H(i) to annihilate
!              A(i+2:n,i)
!
               nik8=1; CALL DLARFG( N-I, A( I+1, I ), A( MIN( I+2, N ), I ), nik8, TAU( I ) )
               E( I ) = A( I+1, I )
               A( I+1, I ) = ONE
!
!              Compute W(i+1:n,i)
!
     ...
     &                     A( I+1, I ), nik8, ZERO, W( I+1, I ), nik8 )
               CALL DGEMV( 'Transpose', N-I, I-1, ONE, W( I+1, 1 ), LDW, &
     &                     A( I+1, I ), nik8, ZERO, W( 1, I ),nik8 )
               CALL DGEMV( 'No transpose', N-I, I-1, -ONE, A( I+1, 1 ), &
     &                     LDA, W( 1, I ), nik8, ONE, W( I+1, I ), nik8 )
               CALL DGEMV( 'Transpose', N-I, I-1, ONE, A( I+1, 1 ), LDA, &
     &                     A( I+1, I ), nik8, ZERO, W( 1, I ), nik8 )
               CALL DGEMV( 'No transpose', N-I, I-1, -ONE, W( I+1, 1 ), &
     &                     LDW, W( 1, I ), nik8, ONE, W( I+1, I ), nik8 )
               CALL DSCAL( N-I, TAU( I ), W( I+1, I ), nik8 )
               ALPHA = -HALF*TAU( I )*DDOT( N-I, W( I+1, I ), nik8, A( I+1, I ), nik8 )
               CALL DAXPY( N-I, ALPHA, A( I+1, I ), nik8, W( I+1, I ), nik8)
            END IF
!
   20    CONTINUE
      END IF
...
!
      nik8 = LEND-L+1;  ANORM = DLANST( 'I', nik8, D( L ), E( L ) )
      ISCALE = 0
...
!
         IF( M.EQ.L+1 ) THEN
            IF( ICOMPZ.GT.0 ) THEN
               ...
               WORK( L ) = C
               WORK( N-1+L ) = S
               nk8 = 2; CALL DLASR( 'R', 'V', 'B', N, nk8, WORK( L ), WORK( N-1+L ), Z( 1, L ), LDZ )
            ELSE
...
!
         IF( M.EQ.L-1 ) THEN
            IF( ICOMPZ.GT.0 ) THEN
               ...
               WORK( M ) = C
               WORK( N-1+M ) = S
               nk8 = 2; CALL DLASR( 'R', 'V', 'F', N, nk8, WORK( M ), WORK( N-1+M ), Z( 1, L-1 ), LDZ )
            ELSE
...
!
  140 CONTINUE
      IF( ISCALE.EQ.1 ) THEN
         nik8 = 0; nk8 = 1; CALL DLASCL( 'G', nik8, nik8, SSFMAX, ANORM, LENDSV-LSV+1, nk8, D( LSV ), N, INFO )
         nik8 = 0; nk8 = 1; CALL DLASCL( 'G', nik8, nik8, SSFMAX, ANORM, LENDSV-LSV,   nk8, E( LSV ), N, INFO )
      ELSE IF( ISCALE.EQ.2 ) THEN
         nik8 = 0; nk8 = 1; CALL DLASCL( 'G', nik8, nik8, SSFMIN, ANORM, LENDSV-LSV+1, nk8, D( LSV ), N, INFO )
         nik8 = 0; nk8 = 1; CALL DLASCL( 'G', nik8, nik8, SSFMIN, ANORM, LENDSV-LSV,   nk8, E( LSV ), N, INFO )
      END IF
!
...
!
  ...
  170       CONTINUE
            IF( K.NE.I ) THEN
               D( K ) = D( I )
               D( I ) = P
               nik8 = 1; CALL DSWAP( N, Z( 1, I ), nik8, Z( 1, K ), nik8 )
            END IF
  180    CONTINUE
      END IF
!
