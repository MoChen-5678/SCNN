...
class DiracNet(nn.Module):
    ...
    def __init__(self, n_hidden=None, n_layers=None, activation='swish',
                 ...
                 hard_g_endpoint_zeros=True,
                 learn_boundary_c=True,
                 use_boundary_phase=False,
                 boundary_phi_init=0.0):
        super().__init__()
        ...
        self.n_hidden = nh
        self.n_layers = nl
        self.use_boundary_factor = use_boundary_factor
        self.hard_g_endpoint_zeros = hard_g_endpoint_zeros
        self.learn_boundary_c = learn_boundary_c
        self.use_boundary_phase = use_boundary_phase
        ...
        # ─── 可学习边界相位修正参数 ───
        # c: exp(i k c r) 中的可学习因子, 约束在 [0.95, 1.05]。
        #    k = sqrt(E(E+2M)) / ħc, 其中 E 使用当前能量估计 (MeV)。
        self.boundary_c_raw = nn.Parameter(torch.tensor([0.0], dtype=torch.float32))
        self.boundary_c_raw.requires_grad_(learn_boundary_c)
        self.boundary_phi = nn.Parameter(torch.tensor([boundary_phi_init], dtype=torch.float32))
        self.boundary_phi.requires_grad_(use_boundary_phase)
    ...
    def forward(self, r, kappa=None, dr=0.10, boundary_energy=None):
        """
        前向传播: r → (G, F)
        ...
        参数:
            ...
            boundary_energy: 可选, 用于边界因子 k=sqrt(E(E+2M)) 的能量 (MeV)。
                             None 时使用网络内部 self.E。
        ...
        ★ 硬边界约束 (RHF论文 + 核物理教材):
          ...
          - r→∞: ψ~Re[e^{i k c r}], c可学习 → 正能为 cos(kcr), 束缚态为指数衰减
        """
        ...
        # 5. 硬边界约束: 乘以渐近因子
        if kappa is not None and self.use_boundary_factor:
            g, f = self._apply_boundary_factors(g, f, r, kappa, boundary_energy)

        # 6. 硬归一化
        if self.hard_normalize:
            g, f = self._normalize(g, f, dr)

        # 7. 端点硬边界: 只控制 r=0 和 r=R 两个网格点的 G=0
        if self.hard_g_endpoint_zeros:
            g = self._enforce_g_endpoint_zeros(g)

        # 8. 相位对齐: 确保 G 的主峰为正
        g, f = self._align_phase(g, f, dr)

        # ★ 9. 内嵌 Gram-Schmidt 正交化 (对所有参考波函数做硬正交投影)
    ...
    def _enforce_g_endpoint_zeros(self, g):
        """硬嵌入 G(r=0)=0 和 G(r=R)=0，只改首尾两个网格点。"""
        if g.shape[-1] < 2:
            return g
        g_bc = g.clone()
        g_bc[..., 0] = 0.0
    ...
    def _apply_boundary_factors(self, g, f, r, kappa, boundary_energy=None):
        """
        硬边界约束: 乘以解析渐近因子。
        ...
        ★ r→∞ 渐近相位/衰减因子:
          ψ(r) ~ Re[e^{i k c r}], k = sqrt(E(E+2M)) / ħc
          c ∈ [0.95, 1.05] 为可学习参数。
          当 E(E+2M) >= 0 时取 cos(kcr)；当 E(E+2M) < 0 时 k 为虚数，
          Re[e^{i k c r}] 等价为 exp(-|k|cr)，保留束缚态尾部衰减。

        ★ 完整拟设: G(r) = r^{l_u+1} · N_net(r) · Re[e^{i k c r}]
        """
        ...
        factor_g = r_safe
        factor_f = r_safe
        boundary_factor = self._boundary_energy_factor(r_safe, boundary_energy)

        g = g * factor_g * boundary_factor
        f = f * factor_f * boundary_factor

        return g, f

    def get_boundary_c(self):
        """返回约束在 [0.95, 1.05] 的可学习边界参数 c。"""
        if not self.learn_boundary_c:
            return torch.ones_like(self.boundary_c_raw)
        return 0.95 + 0.10 * torch.sigmoid(self.boundary_c_raw)

    def get_boundary_phi(self):
        """返回边界振荡因子的相位修正 phi。"""
        if not self.use_boundary_phase:
            return torch.zeros_like(self.boundary_phi)
        return self.boundary_phi

    def _boundary_energy_factor(self, r_safe, boundary_energy=None):
        """计算 Re[exp(i k c r)] 的实数形式。"""
        c = self.get_boundary_c().to(device=r_safe.device, dtype=r_safe.dtype)
        phi = self.get_boundary_phi().to(device=r_safe.device, dtype=r_safe.dtype)
        if boundary_energy is None:
            E = self.E.to(device=r_safe.device, dtype=r_safe.dtype)
        elif isinstance(boundary_energy, torch.Tensor):
            E = boundary_energy.to(device=r_safe.device, dtype=r_safe.dtype)
        else:
            E = torch.as_tensor(boundary_energy, device=r_safe.device, dtype=r_safe.dtype)
        mass = torch.as_tensor(M_NUCLEON, device=r_safe.device, dtype=r_safe.dtype)
        q = E * (E + 2.0 * mass)
        if r_safe.dim() > 1 and q.dim() == 1 and q.numel() == r_safe.shape[0]:
            q = q.unsqueeze(-1)

        k_real = torch.sqrt(torch.clamp(q, min=0.0)) / HBAR_C
        k_imag = torch.sqrt(torch.clamp(-q, min=0.0)) / HBAR_C

        oscillatory = torch.cos(k_real * c * r_safe + phi)
        decaying = torch.exp(-k_imag * c * r_safe)
        return torch.where(q >= 0.0, oscillatory, decaying)

    def boundary_phase_loss(self, r, boundary_energy=None):
        """
        相位边界约束: 对正能量非束缚态约束 cos(k c R + phi)=0。

        该项只在启用 use_boundary_phase 时生效；束缚态(q<0)返回0。
        """
        if not self.use_boundary_phase:
            return torch.tensor(0.0, device=r.device, dtype=r.dtype)

        r_edge = r[..., -1:]
        c = self.get_boundary_c().to(device=r.device, dtype=r.dtype)
        phi = self.get_boundary_phi().to(device=r.device, dtype=r.dtype)
        if boundary_energy is None:
            E = self.E.to(device=r.device, dtype=r.dtype)
        elif isinstance(boundary_energy, torch.Tensor):
            E = boundary_energy.to(device=r.device, dtype=r.dtype)
        else:
            E = torch.as_tensor(boundary_energy, device=r.device, dtype=r.dtype)

        mass = torch.as_tensor(M_NUCLEON, device=r.device, dtype=r.dtype)
        q = E * (E + 2.0 * mass)
        k_real = torch.sqrt(torch.clamp(q, min=0.0)) / HBAR_C
        residual = torch.cos(k_real * c * r_edge + phi).pow(2)
        return torch.where(q >= 0.0, residual, torch.zeros_like(residual)).mean()
